...
bool generate_cam_mask(const cv::Matx33f& K,
    ...
    for (int i = 0; i < 2; ++i)
    {
        ...
        for (int deg = 30; deg < 90; deg += 5){
            //从中心点发出与光心夹角为deg的线段，线段的z是1
            float d = std::tan(deg * M_PI / 180.f);//相机坐标系是一般是右下前,这里射线假设z是1,采样的是fov的一半的射线角度 //射线段在相机平面的投影d
            float a = direction * d * cos(theta);//d的x轴坐标
            float b = direction * d * sin(theta);//d的y轴坐标
            std::vector<cv::Vec3f> ray(1);
            std::vector<cv::Point2f> dist_pt;
            ray[0] = cv::Vec3f(a, -b, 1);
            if(!fisheye)
                //此时的世界坐标系与相机坐标系是重合的，即点(0,0,1)会投影到(cy,cx)上，所以projectPoints中的R,t都是空
                cv::projectPoints(ray, cv::Vec3f(), cv::Vec3f(), K, dist_coeffs, dist_pt);//将点重投影回图像上
            else
            {
            ...
            if (dist_pt[0].x > 0 &&
                ...
                break;//如果超出图像边界了，那就说明那个角度的射线是看不到的，也就是视场角并没有那么大，该结束了
            }
        }
    }

    if (viewable_degs[0] == 0 || viewable_degs[1] == 0) {//左下角与右上角射线的夹角就是视场角了
    ...
    *fov_deg = viewable_degs[0] + viewable_degs[1];//左右各能看到多少度，加起来就是视场角
    ...
    if(!fisheye)
        //将一半视场角的三维点投影回图像平面，投影得到的线段长度为r,以图像中心点r为半径画圆
...
bool detect_orb_features(const cv::Mat& img_in_smooth,
  ...
  if (enforce_uniformity_radius > 5 &&
    ...
    vector<ScoreCalculator<float>::PointWithScore> points;
