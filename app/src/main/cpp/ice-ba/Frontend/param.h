...
class DuoCalibParam : public ParamBase {
 public:
  ...
  struct Imu_t {
    ...
    Eigen::Vector3f gyro_bias;//角速度偏置
    Eigen::Vector3f accel_noise_var;  // m / sec^2
    Eigen::Vector3f angv_noise_var;  // rad / sec
    Eigen::Matrix4f D_T_I;//imu到左相机的变换矩阵
    Eigen::Matrix4f undist_D_T_I;
  } Imu;

  struct Camera_t {
    std::vector<Eigen::Matrix4f, Eigen::aligned_allocator<Eigen::Matrix4f>> D_T_C_lr;//左右相机到相机坐标系(左相机)下的变换矩阵
    std::vector<Eigen::Matrix3f, Eigen::aligned_allocator<Eigen::Matrix3f>> cameraK_lr;//eigen形式左右相机内参
    std::vector<cv::Matx33f> cv_camK_lr;//cv形式左右相机内参
    std::vector<cv::Mat_<float>> cv_dist_coeff_lr;
    // the boundary of images in uv coordinate
    std::vector<Eigen::Vector4f, Eigen::aligned_allocator<Eigen::Vector4f>> lurd_lr;//分别是左右相机的边界,顺序是左x上y右x下y
