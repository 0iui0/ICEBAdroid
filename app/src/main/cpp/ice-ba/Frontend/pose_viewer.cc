...
PoseViewer::PoseViewer() :
  ...
  _image.create(imageSize, imageSize, CV_8UC4);
}
...
void PoseViewer::getTrajectoryImg(cv::Mat &src){
  src = _image;
}
bool PoseViewer::drawTo(cv::Mat* img_ptr) {
  ...
  cv::Mat& img = *img_ptr;
  cv::Scalar backG(255,255,255);
  cv::Scalar textC(0,0,0);
  ...
  // Display text for trajectory 0
  {
    ...
    cv::putText(img, postext.str(), cv::Point(15, 15),
                cv::FONT_HERSHEY_COMPLEX, 0.5, textC, 1);
