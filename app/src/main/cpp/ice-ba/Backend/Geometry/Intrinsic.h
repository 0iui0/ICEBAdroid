...
//获取矫正图像后，矫正的图像的fx,fy
//对图像边缘点去畸变
//去除畸变后的边缘点围成的大小就是矫正图像的大小，这时图像的fx,fy需要乘图像缩放的系数
  inline float GetRectifiedFocal() const {
    const float xdMax = m_k.m_cx / m_k.m_fx;//主目的内参cx/fx  x=(u-cx)/fx,u=0的时候那么就等于-xdMax，u=2cx的时候就是xdMax
...
//#if 1
    ...
    for (int i = 0; i < 4; ++i) {
      if (m_fishEye){
        switch (i) {//xd 图像上四个角落的像素点对应的归一化坐标
        case 0: xd.Set( 0,  -ydMax); break;//上
        case 1: xd.Set( xdMax, 0); break;//右
        case 2: xd.Set(-0,  ydMax); break;//下
        case 3: xd.Set(-xdMax, 0); break;//左
      }
    }
    else{
      switch (i) {//xd 图像上四个角落的像素点对应的归一化坐标
      case 0: xd.Set( xdMax,  ydMax); break;//右下
      case 1: xd.Set( xdMax, -ydMax); break;//右上
      case 2: xd.Set(-xdMax,  ydMax); break;//左下
      case 3: xd.Set(-xdMax, -ydMax); break;//左下
      }
    }

      if (!Undistort(xd, &xn)) {//xn去除畸变之后的归一话坐标，鱼眼用这几个坐标可能问题有点儿大，感觉鱼眼应该使用四边的中点
        continue;
      }
#if 0
//#if 1
...
//#if 1
  ...
  bool undistortPoint(const Point2D &xd, Point2D *xn, const Parameter &instrinc) const;
 protected:
